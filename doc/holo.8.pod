...
Holo can be instructed to operate on a directory other than the root directory.
Some plugins may need to talk to system daemons to operate fully, but they
should restrain from doing so if this variable is present and set to a value
other than F</>.

=item C<$TMPDIR> (default: F</tmp>)

Where to place temporary files; this does not have to be below
C<$HOLO_ROOT_DIR>.
...
=item F<$HOLO_ROOT_DIR/etc/holorc> (see L<holorc(5)>)

=item F<$HOLO_ROOT_DIR/etc/holorc.d/*> (see L<holorc(5)>)

=item F<$HOLO_ROOT_DIR/run/holo.pid>
...
=item F<$HOLO_ROOT_DIR/usr/share/holo/$PLUGIN_ID/> (must exist)

=item F<$HOLO_ROOT_DIR/var/lib/holo/$PLUGIN_ID/> (will be created if does not exist)
